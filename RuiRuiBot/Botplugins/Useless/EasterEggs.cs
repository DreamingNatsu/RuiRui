...
using RuiRuiBot.Botplugins.Triggers;
...
namespace RuiRuiBot.Botplugins.Useless
{
    public class EasterEggs : IModule {
        ...
        public void Install(ModuleManager modman){
            ...
            modman.CreateCommands(bot =>
            {
                bot.CreateCommand("addeasteregg")
                    .Description("Adds an easter egg trigger").MinPermissions((int) Roles.Triumvirate)
                    .Parameter("trigger")
                    .Parameter("response")
                    .Parameter("type")
                ...
                bot.CreateCommand("listeggs").Do((m, db) => Enumerable.ToList<EasterEgg>(db.EasterEggs).Select(ee => $"{ee.Trigger}: {ee.ResponseFormat}\n").ToArray());

                modman.MessageReceived += modman.TryEvent<MessageEventArgs>(async (s, m) =>
                {
        ...
        private static async Task SendResponse(EasterEgg ee, MessageEventArgs m)
        {
            switch (ee.ResponseType)
            {
                case ResponseType.String:
                    await m.Channel.SendBigMessage(string.Format(ee.ResponseFormat, m.Message.User.Name));
                    break;
