...
namespace Logic {
    ...
    public class RssTorrentManager : IDisposable {
        ...
        public void UpdateTorrentModels(){
            using (var tss = new TransmissionService()) {
                _torrents = tss.GetTorrents();
                if (_torrents.Count <= 0) return;
                using (var db = new DbCtx()) {
                    var returnList = new List<RssTorrentModel>();
                    foreach (
                        var item in
                            db.RssTorrents.Select(
                                rsstorrent =>
                                    new RssTorrentModel{
                                        Id = rsstorrent.Id,
                                        Path = rsstorrent.Path,
                                        RssUrl = rsstorrent.RssUrl
                                    })) {
        ...
        private List<ListedTorrent> GetRss(RssTorrentModel item){
            ...
            foreach (var descendant in document.Descendants("item"))
                try {
                    var xDescription = descendant.Element("description");
                    var xTitle = descendant.Element("title");
                    var xDate = descendant.Element("pubDate");
                    if (xDescription == null || xTitle == null || xDate == null) continue;
                    list.Add(new ListedTorrent{
                        Description = xDescription.Value,
                        Title = xTitle.Value,
                        Date = DateTime.Parse(xDate.Value),
                        Status = GetTorrentItem(xTitle.Value)
                    });
                }
                catch (Exception e) {
                    list.Add(ErrorTorrent(e));
